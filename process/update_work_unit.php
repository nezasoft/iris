...
//check if its an ajax request
 if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
  ...
  $work_unit = $input['WorkUnit'];
	  ...
	  $update = $conn->prepare("UPDATE  work_unit SET work_unit_name='".$work_unit."' WHERE id='".$item_code."'");
	  $update->execute();
	  ...
	  if($row_count >=1){
	   ...
	   $resp = 'Record failed to updated!';
      	 ...
      	 $log_desc ="Updated details for the work unit <strong>".$input['WorkUnit']."</strong>";
		  //Log Activity
		...
		$update = $conn->prepare("DELETE FROM work_unit WHERE id='".$item_code."'  LIMIT 1");
		$resp = 'Record Deleted!';
		//Lets make the necessary changes
		$update->execute();
		$log_desc ="Removed work unit <strong>".$input['WorkUnit']."</strong>";
		//Log Activity
		$log = $conn->prepare("INSERT INTO system_logs(system_date,time_stamp,user_id,log_desc,doc_type,doc_no)VALUES(curdate(),curtime(),'".$_SESSION['IRIS_USER_ID']."','".$log_desc."','OTHER','')");
		$log->execute(); 
