...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	...
	try{
		 ...
		 $check= $conn->prepare("SELECT id FROM sizes WHERE size_name='".$size_name."' LIMIT 1");
		 $check->execute();
		 ...
		 //save data
		 $save = $conn->prepare("INSERT INTO sizes(size_name)VALUES('".$size_name."')");
		 $save->execute();
		 $log_desc ="Added a new size  <strong>".$size_name."</strong>";
		  //Log Activity
