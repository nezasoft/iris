...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	$ref_no = sanitize_string($_POST['ref_no']);
	  ...
	  $inv_qr = $conn->prepare("SELECT document_value FROM business_doc WHERE doc_code='MR'  ORDER BY id DESC LIMIT 1");
      $inv_qr->execute();
	...
	try{
		 ...
		 $check= $conn->prepare("SELECT id FROM materials_request_form WHERE order_no='".$receipt_no."' LIMIT 1");
		 $check->execute();
		...
		//save data
		 $save = $conn->prepare("INSERT INTO materials_request_form(order_no,user_id,mat_id,qty,req_desc,req_date,req_time,building_id,work_id,site_id)VALUES('".$receipt_no."','".$_SESSION['IRIS_USER_ID']."','".$row['mat_id']."','".$row['qty']."','".$row['req_details']."',curdate(),curtime(),'".$row['building_id']."','".$row['work_id']."','".$_SESSION['IRIS_SITE_ID']."')");
		 $save->execute();			
		}
		
		 $log_desc ="Created a new material requisition. Document no.  <strong>".$receipt_no."</strong>";
		  //Log Activity
		  ...
		  $log->execute();
		  $doc_value = $conn->prepare("UPDATE business_doc set document_value = document_value + 1 WHERE doc_code='MR' LIMIT 1");
		  $doc_value->execute();
		  ...
		  $empty_table->execute();
			 ...
			 $notif_title = "Material Requisition Request";
			 $notif_msg = "You have a new material requisition request waiting for your approval";
			 ...
			 $send_notif->execute();
			 $subject = 'Material Requisition Approval Request - Order No -'.$receipt_no;
