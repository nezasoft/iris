...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	...
	try{
		 ...
		 $check= $conn->prepare("SELECT id FROM packages WHERE pack_name='".$package_name."' LIMIT 1");
		 $check->execute();
		 ...
		 //save data
		 $save = $conn->prepare("INSERT INTO packages(pack_name)VALUES('".$package_name."')");
		 $save->execute();
		 $log_desc ="Added a new size  <strong>".$package_name."</strong>";
		  //Log Activity
