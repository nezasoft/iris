...
//check if its an ajax request
	if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
		...
		$stmt = $conn->prepare("SELECT st.id AS item_id, st.min_stock_level,st.max_stock_level, st.quantity,st.stock_desc, mt.mat_name,pk.pack_name   FROM materials_stock AS st LEFT JOIN stores AS s ON s.id = st.store_id LEFT JOIN materials AS mt ON mt.id = st.mat_id LEFT JOIN packages AS pk ON pk.id = st.pack_id WHERE s.site_id='".$site_id."' AND (mt.mat_name LIKE '%".$search."%' OR st.stock_desc LIKE '%".$search."%') ORDER BY mt.mat_name ASC  LIMIT 30");
