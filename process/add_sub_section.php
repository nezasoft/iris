...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	...
	$sub_section = sanitize_string($_POST['sub_section_name']);	
	try{
		 $conn->beginTransaction();
		 //check if record exists
		 $check= $conn->prepare("SELECT id FROM sub_section WHERE sub_section_name='".$sub_section."' AND section_id='".$section."' LIMIT 1");
		 $check->execute();
		 ...
		 //save data
		 $save = $conn->prepare("INSERT INTO sub_section(sub_section_name,section_id)VALUES('".$sub_section."','".$section."')");
		 $save->execute();
		 $log_desc ="Added a new sub section   <strong>".$sub_section."</strong>";
		  //Log Activity
