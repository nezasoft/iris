...
//check if its an ajax request
 if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
  ...
  $sub_section = $input['SubSection'];
   	if($input['action']==='edit'){	 
	  $update = $conn->prepare("UPDATE sub_section SET sub_section_name='".$sub_section."', section_id='".$section."' WHERE id='".$item_code."'");
	  $update->execute();
	  ...
	  if($row_count >=1){
	   ...
	   $resp = 'Record failed to updated!';
      	 ...
      	 $log_desc ="Updated details for the sub section <strong>".$input['SubSection']."</strong>";
		  //Log Activity
		...
		$update = $conn->prepare("DELETE FROM sub_section WHERE id='".$item_code."'  LIMIT 1");
		$resp = 'Record Deleted!';
		//Lets make the necessary changes
		$update->execute();
		$log_desc ="Removed sub section <strong>".$input['SubSection']."</strong>";
		//Log Activity
		$log = $conn->prepare("INSERT INTO system_logs(system_date,time_stamp,user_id,log_desc,doc_type,doc_no)VALUES(curdate(),curtime(),'".$_SESSION['IRIS_USER_ID']."','".$log_desc."','OTHER','')");
		$log->execute(); 		
	}	
