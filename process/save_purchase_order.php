...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	...
	$req = sanitize_string($_POST['req']);
	  ...
	  $inv_qr = $conn->prepare("SELECT document_value FROM business_doc WHERE doc_code='PO'  ORDER BY id DESC LIMIT 1");
      $inv_qr->execute();
	...
	try{
		 ...
		 $check= $conn->prepare("SELECT id FROM purchase_order WHERE order_no='".$receipt_no."' LIMIT 1");
		 $check->execute();
		...
		$get_data = $conn->prepare("SELECT id AS item_id, ref_no,mat_id,rate,qty,order_details FROM temp_purchase_order  AS tr WHERE ref_no='".$_SESSION['IRIS_PO_REF_NO']."' ORDER BY id ASC");
		$get_data->execute();
		$get_data_rows = $get_data->fetchAll(PDO::FETCH_ASSOC);
		
		foreach($get_data_rows as $row){
		//save data
		 $save = $conn->prepare("INSERT INTO purchase_order(order_no,user_id,mat_id,qty,order_details,order_date,order_time,rate,site_id,sup_id,req_id)VALUES('".$receipt_no."','".$_SESSION['IRIS_USER_ID']."','".$row['mat_id']."','".$row['qty']."','".str_replace("'","&sbquo;",$row['order_details'])."',curdate(),curtime(),'".$row['rate']."','".$site."','".$supplier."','".$req."')");
		 $save->execute();			
		}
		
		 $log_desc ="Created a new Purchase Order. Document no.  <strong>".$receipt_no."</strong>";
		  //Log Activity
		  ...
		  $log->execute();
		  $doc_value = $conn->prepare("UPDATE business_doc set document_value = document_value + 1 WHERE doc_code='PO' LIMIT 1");
		  $doc_value->execute();
		  //lets empty the temp table
		  $empty_table = $conn->prepare("DELETE FROM temp_purchase_order WHERE ref_no='".$_SESSION['IRIS_PO_REF_NO']."'");
		  $empty_table->execute();
		 //Get Approval persons details
			 $get_app = $conn->prepare("SELECT fname, email, id AS sender_id  FROM users WHERE site_id='".$_SESSION['IRIS_SITE_ID']."' AND (role_id=6 OR role_id=13) AND active='Yes'");
			 $get_app->execute();
			 ...
			 $notif_title = "LPO  Request";
			 $notif_msg = "You have a new LPO  request waiting for your approval";
			 ...
			 $subject = 'Purchase Order Approval Request - Order No -'.$receipt_no;
			 $message = " Dear ".titleCase($fname).",<br/> Your approval for an LPO is needed. Please log into the system to approve. Order no. <strong>".$receipt_no."</strong><br/>
			 Kind Regards,<br/>
