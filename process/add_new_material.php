...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	$cat = sanitize_string($_POST['cat']);
	$brand = sanitize_string($_POST['brand']);
	$size = sanitize_string($_POST['size']);
	$sup = sanitize_string($_POST['sup']);
	$man = sanitize_string($_POST['man']);
	$item_name = sanitize_string($_POST['item_name']);
	$remark = sanitize_string($_POST['remark']);	
	try{
		 $conn->beginTransaction();
		 //check if record exists
		 $check= $conn->prepare("SELECT id FROM materials WHERE mat_name='".$item_name."' AND brand_id='".$brand."' AND size_id='".$size."' LIMIT 1");
		 $check->execute();
		 ...
		 //save data
		 $save = $conn->prepare("INSERT INTO materials(mat_name,sup_id,description,mat_cat_id,user_id,man_id,brand_id,size_id)VALUES('".$item_name."','".$sup."','".$remark."','".$cat."','".$_SESSION['IRIS_USER_ID']."','".$man."','".$brand."','".$size."')");
		 $save->execute();
		 $log_desc ="Add new material  <strong>".$item_name."</strong>";
		//Log Activity
