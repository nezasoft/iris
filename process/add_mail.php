...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	$mailer_name = sanitize_string($_POST['mailer_name']);
	$ip= sanitize_string($_POST['mailer_ip']);
	$incoming = sanitize_string($_POST['incoming']);
	$outgoing  = sanitize_string($_POST['outgoing']);
	$sender = sanitize_string($_POST['sender']);
	$username = sanitize_string($_POST['username']);
	$password = sanitize_string($_POST['password']);	
	if(!filter_var($sender, FILTER_VALIDATE_EMAIL)){		
		die("<div class='alert alert-danger'>* Invalid Email Address</div>");				
	}
	try{
		 $conn->beginTransaction();
		 //check if record exists
		 $check= $conn->prepare("SELECT id FROM mail_setting WHERE mailer_name='".$mailer_name."' AND mail_url='".$ip."' LIMIT 1");
		 $check->execute();
		 ...
		 //save data
		 $save = $conn->prepare("INSERT INTO mail_setting(mailer_name,mail_url,incoming_port,outgoing_port,email,username,password)VALUES('".$mailer_name."','".$ip."','".$incoming."','".$outgoing."','".$sender."','".$username."','".$password."')");
		 $save->execute();
		 $log_desc ="Added a new mail server  <strong>".$mailer_name."</strong>";
		  //Log Activity
