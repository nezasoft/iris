...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	...
	try{
		 ...
		 $check= $conn->prepare("SELECT id FROM boq_items WHERE boq_id='".$ref_no."' AND item_id='".$material."'  AND work_id='".$work_unit."'  LIMIT 1");
		 $check->execute();
		 ...
		 //save data
		 $save = $conn->prepare("INSERT INTO boq_items(item_id,boq_id,remark,qty,rate,work_id)VALUES('".$material."','".$ref_no."','".$remark."','".$quantity."','".$rate."','".$work_unit."')");
		 $save->execute();
		 $conn->commit();
