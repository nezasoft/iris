...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
	...
	if(in_array($file_ext,$allowed_file_types) && ($filesize < 5000000)){	
		...
		if(file_exists("../req_files/".$newfilename)){
			...
			move_uploaded_file($_FILES["file"]["tmp_name"], "../req_files/" . $newfilename);
			//save image in DB
			$save = mysql_query("UPDATE requisitions SET file_name='".$newfilename."' WHERE req_ref_no='".$ref_no."'");
