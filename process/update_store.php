...
//check if its an ajax request
 if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
  ...
  $desc = sanitize_string($input['Desc']);
   	if($input['action']==='edit'){	 
	  $update = $conn->prepare("UPDATE  stores SET store_name='".$name."',store_desc='".$desc."' WHERE id='".$item_code."'");
	  $update->execute();
	  ...
	  if($row_count >=1){
	   ...
	   $resp = 'Record failed to updated!';
		...
		$update = $conn->prepare("DELETE FROM stores WHERE id='".$item_code."'  LIMIT 1");
		$resp = 'Record Deleted!';
