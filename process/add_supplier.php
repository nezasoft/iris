...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	...
	$sup_physical = sanitize_string($_POST['sup_physical']);	
	try{
		 $conn->beginTransaction();
		 //check if record exists
		 $check= $conn->prepare("SELECT id FROM suppliers WHERE sup_name='".$sup_name."' LIMIT 1");
		 $check->execute();
		 ...
		 //save data
		 $save = $conn->prepare("INSERT INTO suppliers(sup_name,sup_email,sup_phone,sup_postal,sup_physical)VALUES('".$sup_name."','".$sup_email."','".$sup_phone."','".$sup_postal."','".$sup_physical."')");
		 $save->execute();
		 $log_desc ="Added a new supplier   <strong>".$sup_name."</strong>";
		  //Log Activity
