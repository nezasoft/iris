...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	...
	$_SESSION['IRIS_DEL_REF_NO'] = $ref_no;
	try{
		 $conn->beginTransaction();
		 //check if record exists
		 $check= $conn->prepare("SELECT id FROM temp_delivery_note WHERE ref_no='".$ref_no."' AND mat_id='".$material."'   LIMIT 1");
		 $check->execute();
		 ...
		 //save data
		 $save = $conn->prepare("INSERT INTO temp_delivery_note(mat_id,ref_no,del_details,qty)VALUES('".$material."','".$ref_no."','".$remark."','".$quantity."')");
		 $save->execute();
		 $conn->commit();
