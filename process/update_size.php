...
//check if its an ajax request
 if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
  ...
  $item_name = $input['SizeName'];
   	if($input['action']==='edit'){	 
	  $update = $conn->prepare("UPDATE  sizes SET size_name='".$item_name."' WHERE id='".$item_code."'");
	  $update->execute();
	  ...
	  if($row_count >=1){
	   ...
	   $resp = 'Record failed to updated!';
      	 ...
      	 $log_desc ="Updated details for the size <strong>".$input['SizeName']."</strong>";
		  //Log Activity
		...
		$update->execute();
		$log_desc ="Removed sizes <strong>".$input['SizeName']."</strong>";
		//Log Activity
		$log = $conn->prepare("INSERT INTO system_logs(system_date,time_stamp,user_id,log_desc,doc_type,doc_no)VALUES(curdate(),curtime(),'".$_SESSION['IRIS_USER_ID']."','".$log_desc."','OTHER','')");
		$log->execute(); 
