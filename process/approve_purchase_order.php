...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	...
	try{
		 ...
		 $check= $conn->prepare("SELECT id FROM lpo_approvals WHERE ref_no='".$order_no."' AND user_id='".$_SESSION['IRIS_USER_ID']."'  LIMIT 1");
		 $check->execute();
		 $check_count = $check->rowCount();		 
		 if($check_count==1){
			 die("<div class='alert alert-danger'> * Request already actioned on !</div>");			 
		 }		 
		 //save data
		 $save = $conn->prepare("INSERT INTO lpo_approvals(ref_no,user_id,approve_date,approve_time,approval_status,remarks)VALUES('".$order_no."','".$_SESSION['IRIS_USER_ID']."',curdate(),curtime(),'".$approval."','".$remark."')");
		 $save->execute();	
		 //lets change the status of the document
		 $update = $conn->prepare("UPDATE purchase_order SET order_status='".$approval."' WHERE order_no='".$order_no."'");
		 $update->execute();	 
		 $log_desc ="Actioned on purchase order no:   <strong>".$order_no."</strong>";
		  //Log Activity
