...
//check if its an ajax request
 if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
  ...
  $remarks = $input['Remarks'];
	  ...
	  $update = $conn->prepare("UPDATE  boq_temp SET qty='".$qty."',remark='".$remarks."',rate='".$rate."' WHERE id='".$item_code."'");
	  $update->execute();
	  ...
	  if($row_count>=1){
	   ...
	   $resp = 'Record failed to updated!';
		...
		$update = $conn->prepare("DELETE FROM boq_temp WHERE id='".$item_code."'  LIMIT 1");
		$update->execute();
		$resp = 'Record Deleted!';				
	}	
