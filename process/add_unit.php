...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	$unit_name = sanitize_string($_POST['unit_name']);
	$site = sanitize_string($_POST['site']);	
	try{
		 $conn->beginTransaction();
		 //check if record exists
		 $check= $conn->prepare("SELECT id FROM building WHERE building_name='".$unit_name."' AND site_id='".$site."' LIMIT 1");
		 $check->execute();
		 ...
		 //save data
		 $save = $conn->prepare("INSERT INTO building(building_name,site_id)VALUES('".$unit_name."','".$site."')");
		 $save->execute();
		 $log_desc ="Added a new unit   <strong>".$unit_name."</strong>";
		  //Log Activity
