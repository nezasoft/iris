...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	$level_id = sanitize_string($_POST['level_id']);
	$user_roles = sanitize_string($_POST['user_roles']);
	...
	$count_items = count($user_roles);
try{	
	$conn->beginTransaction();
   for($i=0;$i<=$count_items;$i++){
	   if(!empty($user_roles[$i])){			 
		 $check = $conn->prepare("SELECT id FROM role_permissions WHERE role_id='".$level_id."' AND perm_id='".$user_roles[$i]."' LIMIT 1");
		 $check->execute();
		 $check_count=$check->rowCount();		 
		 if($check_count==0){
			  //save record
		 $save = $conn->prepare("INSERT INTO role_permissions(role_id,perm_id)VALUES('".$level_id."','".$user_roles[$i]."')");
		 $save->execute();			 
		 }
	   }	   	   
   }
	  ...
	  echo '<div class="alert alert-success d-flex align-items-center p-5 mb-10">
	   ...
	   $log_desc ="Assigned permissions  to  <strong>".$role_name."</strong>";
		  //Log Activity
