...
//check if its an ajax request
 if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
  ...
  $remarks = $input['Remarks'];
	  ...
	  $update = $conn->prepare("UPDATE  temp_delivery_note SET qty='".$qty."',del_details='".$remarks."' WHERE id='".$item_code."'");
	  $update->execute();
	  ...
	  if($row_count>=1){
	   ...
	   $resp = 'Record failed to updated!';
		...
		$update = $conn->prepare("DELETE FROM temp_delivery_note WHERE id='".$item_code."'  LIMIT 1");
		$update->execute();
		$resp = 'Record Deleted!';				
	}	
