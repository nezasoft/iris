...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	$section = sanitize_string($_POST['sub_section']);
	$work_unit = sanitize_string($_POST['work_unit']);	
	try{
		 $conn->beginTransaction();
		 //check if record exists
		 $check= $conn->prepare("SELECT id FROM work_unit WHERE work_unit_name='".$work_unit."' AND sub_section_id='".$section."' LIMIT 1");
		 $check->execute();
		 ...
		 //save data
		 $save = $conn->prepare("INSERT INTO work_unit(work_unit_name,sub_section_id)VALUES('".$work_unit."','".$section."')");
		 $save->execute();
		 $log_desc ="Added a new work unit   <strong>".$work_unit."</strong>";
		  //Log Activity
		 ...
		 //reload user reports
		 echo "<script>$('#form_section').empty();getdata(1);window.setTimeout(function(){$('#ModalContentAddNew').modal('hide');},2000);</script>";	
