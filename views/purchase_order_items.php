...
$get_req = $conn->prepare("SELECT  t.id AS item_id, t.order_details, t.qty, t.ref_no, t.rate, m.mat_name FROM temp_purchase_order AS t LEFT JOIN materials AS m ON m.id = t.mat_id WHERE t.ref_no='".$_SESSION['IRIS_PO_REF_NO']."'");
$get_req->execute();
...
<thead style="background:#0291FA ;color:#fff;font-weight:600;">
  ...
  <tr>
    ...
    <td><?php echo $req_row['order_details']; ?></td>
  </tr>
...
$('#rq_items').Tabledit({
    //Delete PO Item
    url: 'process/update_purchase_order_item.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'id'], editable: [ [2, 'Quantity'],[3, 'Rate'],[4, 'Remarks']]
    }
   , buttons: {
        edit: {
