...
$get_req = $conn->prepare("SELECT bq.id AS item_id, bq.qty, bq.rate, bq.remark, m.mat_name, w.work_unit_name, p.pack_name FROM boq_items AS bq LEFT JOIN materials AS m ON m.id = bq.item_id LEFT JOIN work_unit AS w ON w.id = bq.work_id LEFT JOIN packages AS p ON p.id = m.pack_id WHERE bq.boq_id='".$item_id."'");
$get_req->execute();
...
$('#rq_items').Tabledit({
    //Delete PO Item
    url: 'process/update_boq_item_edited.php', editButton: true, removeButton: true, columns: {
