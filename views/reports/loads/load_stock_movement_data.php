...
exit();
		   ...
		   if(isset($_GET['filter_form_data']) && sanitize_string($_GET['filter_form_data'])=='true'){
			...
			$stmt = $conn->prepare("SELECT tr.id AS item_id  FROM stock_transaction AS tr LEFT JOIN materials_stock AS ms ON ms.id = tr.stock_id LEFT JOIN materials AS mt ON mt.id = ms.mat_id LEFT JOIN packages AS pk ON pk.id = ms.pack_id LEFT JOIN sizes AS s ON s.id = mt.size_id LEFT JOIN users AS us ON us.id = tr.user_id LEFT JOIN stores AS st ON st.id = ms.store_id WHERE st.site_id='".$site_id."' ".$filter_t_type." AND (tr.trans_date>='".$from."' AND tr.trans_date<='".$to."')");
		   }else{
			$stmt = $conn->prepare("SELECT tr.id AS item_id  FROM stock_transaction AS tr LEFT JOIN materials_stock AS ms ON ms.id = tr.stock_id LEFT JOIN materials AS mt ON mt.id = ms.mat_id LEFT JOIN packages AS pk ON pk.id = ms.pack_id LEFT JOIN sizes AS s ON s.id = mt.size_id LEFT JOIN users AS us ON us.id = tr.user_id LEFT JOIN stores AS st ON st.id = ms.store_id WHERE st.site_id='".$site_id."' ORDER BY tr.id DESC");
			}							
            $stmt->execute();
...
site_id = <?php echo $site_id ?>
	...
	var myData = 'page=' + pageno + '&filter=' + filter;
     ...
     url: "views/reports/loads/load_stock_movement_data.php", //Where form data is sent on submission
