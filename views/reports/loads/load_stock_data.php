...
exit();
      ...
      //check if user want all records or has wants a filtered result set
           ...
           $url = '?users&action=user_reports&';
			...
			$stmt = $conn->prepare("SELECT st.id AS item_id   FROM materials_stock AS st LEFT JOIN materials AS mt ON mt.id = st.mat_id LEFT JOIN stores AS s ON s.id = st.store_id LEFT JOIN sites AS stt ON stt.id = s.site_id LEFT JOIN packages AS pk ON pk.id = st.pack_id");
            $stmt->execute();
		...
		return $setPaginate;
            ...
            $pageLimit  = ($page * $setLimit) - $setLimit;
		      ...
		      $stmt = $conn->prepare("SELECT st.id AS item_id,stt.site_name, st.min_stock_level,st.max_stock_level, st.quantity,st.stock_desc, mt.mat_name,pk.pack_name   FROM materials_stock AS st LEFT JOIN materials AS mt ON mt.id = st.mat_id LEFT JOIN packages AS pk ON pk.id = st.pack_id LEFT JOIN stores AS s ON s.id = st.store_id LEFT JOIN sites AS stt ON stt.id = s.site_id   ORDER BY mt.mat_name ASC LIMIT ".$pageLimit.",".$setLimit."" );
              $stmt->execute();
	          $displayed_records=0;
...
filter = <?php  echo $filter; ?>;
	...
	var myData = 'page=' + pageno + '&filter=' + filter;
     ...
     url: "views/reports/loads/load_stock_data.php", //Where form data is sent on submission
