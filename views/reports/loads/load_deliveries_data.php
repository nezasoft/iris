...
exit();
		   ...
		   if(isset($_GET['filter_form_data']) && sanitize_string($_GET['filter_form_data'])=='true'){
			...
			$stmt = $conn->prepare("SELECT d.id AS item_id FROM delivery_note AS d LEFT JOIN materials AS m ON m.id = d.mat_id LEFT JOIN sites AS sf ON sf.id = d.site_id LEFT JOIN sites AS st ON st.id = d.to_site_id LEFT JOIN users AS us ON us.id = d.user_id  GROUP BY d.delivery_no  ORDER BY d.id DESC");
			}							
            $stmt->execute();
		...
		return $setPaginate;
	       ...
	       if(isset($_GET['page'])){
			...
			$stmt = $conn->prepare("SELECT d.id AS item_id, d.delivery_no, d.del_details, d.delivery_name, sf.site_name AS site_from, st.site_name AS site_to, d.del_date, d.delivery_status, us.fname FROM delivery_note AS d LEFT JOIN materials AS m ON m.id = d.mat_id LEFT JOIN sites AS sf ON sf.id = d.site_id LEFT JOIN sites AS st ON st.id = d.to_site_id LEFT JOIN users AS us ON us.id = d.user_id  GROUP BY d.delivery_no    ORDER BY d.id  DESC LIMIT ".$pageLimit.",".$setLimit."" );
			}						
$stmt->execute();
$displayed_records =0;
if($num_rows>=1){
?>
      	<div>
			<div class="table-responsive">
                <table class="table table-row-dashed table-striped table-row-gray-300 align-middle gs-0 gy-4" id="deliveries_data" >
            ...
            ?>
              <tr>  
				  ...
				  <td>
				  <div align="center"><a href="#" class="badge badge-success" data-bs-toggle="modal" data-bs-target="#ModalContentViewItem" onclick="viewDeliveryNote(<?php echo $row['delivery_no']; ?>)"> View</a> 
				  </div>
				  </td>
              </tr>   
            <?php
...
filter = <?php  echo $filter; ?>;
	...
	var myData = 'page=' + pageno + '&filter=' + filter;
     ...
     url: "views/reports/loads/load_deliveries_data.php", //Where form data is sent on submission
