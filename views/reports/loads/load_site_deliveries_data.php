...
exit();
		   ...
		   if(isset($_GET['filter_form_data']) && sanitize_string($_GET['filter_form_data'])=='true'){
			...
			if($site=="ALL"){
			$filter_data = "AND d.to_site_id LIKE '%%'";
			}elseif($site!="ALL"){
			$filter_data = "AND d.to_site_id='".$site."'";
			}		
			
			$stmt = $conn->prepare("SELECT d.id AS item_id FROM delivery_note AS d LEFT JOIN materials AS m ON m.id = d.mat_id LEFT JOIN sites AS sf ON sf.id = d.site_id LEFT JOIN sites AS st ON st.id = d.to_site_id LEFT JOIN users AS us ON us.id = d.user_id WHERE (d.del_date>='".$from."' AND d.del_date<='".$to."') ".$filter_data." AND d.site_id='".$site_id."' GROUP BY d.delivery_no  ORDER BY d.id DESC");
		   }else{
			$stmt = $conn->prepare("SELECT d.id AS item_id FROM delivery_note AS d LEFT JOIN materials AS m ON m.id = d.mat_id LEFT JOIN sites AS sf ON sf.id = d.site_id LEFT JOIN sites AS st ON st.id = d.to_site_id LEFT JOIN users AS us ON us.id = d.user_id WHERE d.site_id='".$site_id."' GROUP BY d.delivery_no  ORDER BY d.id DESC");
			}							
            $stmt->execute();
		...
		return $setPaginate;
	       ...
	       if(isset($_GET['page'])){
		    ...
		    $stmt = $conn->prepare("SELECT d.id AS item_id, d.delivery_no, d.del_details, d.delivery_name, sf.site_name AS site_from, st.site_name AS site_to, d.del_date, d.delivery_status, us.fname FROM delivery_note AS d LEFT JOIN materials AS m ON m.id = d.mat_id LEFT JOIN sites AS sf ON sf.id = d.site_id LEFT JOIN sites AS st ON st.id = d.to_site_id LEFT JOIN users AS us ON us.id = d.user_id WHERE d.site_id='".$site_id."' AND (d.del_date>='".$from."' AND d.del_date<='".$to."') ".$filter_data." GROUP BY d.delivery_no    ORDER BY d.id DESC LIMIT ".$pageLimit.",".$setLimit."" );
			}else{
			$stmt = $conn->prepare("SELECT d.id AS item_id, d.delivery_no, d.del_details, d.delivery_name, sf.site_name AS site_from, st.site_name AS site_to, d.del_date, d.delivery_status, us.fname FROM delivery_note AS d LEFT JOIN materials AS m ON m.id = d.mat_id LEFT JOIN sites AS sf ON sf.id = d.site_id LEFT JOIN sites AS st ON st.id = d.to_site_id LEFT JOIN users AS us ON us.id = d.user_id WHERE d.site_id='".$site_id."' GROUP BY d.delivery_no    ORDER BY d.id  DESC LIMIT ".$pageLimit.",".$setLimit."" );
			}						
$stmt->execute();
$displayed_records =0;
if($num_rows>=1){
      	...
      	<div>
            ...
            ?>
              <tr>  
				  ...
				  <td>
				  <div align="center"><a href="#" class="badge badge-success" onclick="viewDocument(<?php echo $row['delivery_no']; ?>)"> View</a> 
				  </div>
				  </td>
              </tr>   
            <?php
...
site_id = "<?php  echo $site_id; ?>";
	// source of data	
	var myData = 'page=' + pageno + '&filter=' + filter + '&filter_form_data=' + filter_form_data   + '&from=' + from  + '&to=' + to  + '&site=' + site + '&site_id=' + site_id;
	 jQuery.ajax({
     type: "GET", // Post / Get method
     url: "views/reports/loads/load_site_deliveries_data.php", //Where form data is sent on submission
     dataType:"text", // Data type, HTML, json etc.
