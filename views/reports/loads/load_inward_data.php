...
exit();
		   ...
		   if(isset($_GET['filter_form_data']) && sanitize_string($_GET['filter_form_data'])=='true'){
			...
			if($site=="ALL"){
			$filter_data = "AND gi.site_id LIKE '%%'";
			}elseif($site!="ALL"){
			$filter_data = "AND gi.site_id='".$site."'";
			}	
			
			$stmt = $conn->prepare("SELECT gi.id AS item_id FROM good_inward AS gi LEFT JOIN materials AS mt ON mt.id = gi.mat_id LEFT JOIN users AS us ON us.id = gi.user_id LEFT JOIN suppliers AS s ON s.id = gi.sup_id   WHERE (gi.inw_date>='".$from."' AND gi.inw_date<='".$to."') ".$filter_data." GROUP BY gi.inward_no  ORDER BY gi.id DESC");
		   }else{
			$stmt = $conn->prepare("SELECT gi.id AS item_id  FROM good_inward AS gi LEFT JOIN materials AS mt ON mt.id = gi.mat_id LEFT JOIN users AS us ON us.id = gi.user_id LEFT JOIN suppliers AS s ON s.id = gi.sup_id   GROUP BY gi.inward_no  ORDER BY gi.id DESC");
			}							
            $stmt->execute();
		...
		return $setPaginate;
	       ...
	       if(isset($_GET['page'])){
		    ...
		    $stmt = $conn->prepare("SELECT gi.id AS item_id, gi.inward_no, gi.inw_date, gi.inw_time, gi.in_status, gi.delivery_note_no, gi.delivered_by, gi.g_desc, gi.vehicle_no, s.sup_name  FROM good_inward AS gi LEFT JOIN materials AS mt ON mt.id = gi.mat_id LEFT JOIN users AS us ON us.id = gi.user_id LEFT JOIN suppliers AS s ON s.id = gi.sup_id  WHERE (gi.inw_date>='".$from."' AND gi.inw_date<='".$to."') ".$filter_data." GROUP BY gi.inward_no ORDER BY gi.id DESC LIMIT ".$pageLimit.",".$setLimit."" );
			}else{
			$stmt = $conn->prepare("SELECT gi.id AS item_id, gi.inward_no, gi.inw_date, gi.inw_time, gi.in_status, gi.delivery_note_no, gi.delivered_by, gi.g_desc, gi.vehicle_no, s.sup_name  FROM good_inward AS gi LEFT JOIN materials AS mt ON mt.id = gi.mat_id LEFT JOIN users AS us ON us.id = gi.user_id LEFT JOIN suppliers AS s ON s.id = gi.sup_id     GROUP BY gi.inward_no  ORDER BY gi.id  DESC LIMIT ".$pageLimit.",".$setLimit."" );
			}						
$stmt->execute();
$displayed_records =0;
if($num_rows>=1){
?>
      	<div>
			<div class="table-responsive">
                <table class="table table-row-dashed table-striped table-row-gray-300 align-middle gs-0 gy-4" id="inward_data" >
            ...
            ?>
              <tr>  
				  ...
				  <td>
				  <div align="center"><a href="#" class="badge badge-success" data-bs-toggle="modal" data-bs-target="#ModalContentViewItem" onclick="viewInwardNote(<?php echo $row['inward_no']; ?>)"> View</a> 
				  </div>
				  </td>
              </tr>   
            <?php
...
site = "<?php echo $site; ?>";
	// source of data	
	var myData = 'page=' + pageno + '&filter=' + filter + '&site=' + site;
	 jQuery.ajax({
     type: "GET", // Post / Get method
     url: "views/reports/loads/load_inward_data.php", //Where form data is sent on submission
