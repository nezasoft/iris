...
exit();
		   ...
		   if(isset($_GET['filter_form_data']) && sanitize_string($_GET['filter_form_data'])=='true'){
			...
			$supplier = sanitize_string($_GET['supplier']); 
			$site = sanitize_string($_GET['site']); 
			$from = sanitize_string($_GET['from']); 
			$to = sanitize_string($_GET['to']); 
			if($supplier=="ALL"){
			$filter_data = "WHERE po.sup_id LIKE '%%' ";
			}elseif($supplier!="ALL"){
			$filter_data = "WHERE po.sup_id='".$supplier."' ";
			}
			if($site=="ALL"){
			$filter_data .= "AND po.site_id LIKE '%%'";
			}elseif($site!="ALL"){
			$filter_data .= "AND po.site_id='".$site."'";
			}
			
			$stmt = $conn->prepare("SELECT po.id AS item_id FROM purchase_order AS po LEFT JOIN materials AS mt ON mt.id = po.mat_id LEFT JOIN suppliers AS s ON s.id = po.sup_id LEFT JOIN sites AS st ON st.id = po.site_id LEFT JOIN users AS us ON us.id = po.user_id   ".$filter_data." AND (po.order_date>='".$from."' AND po.order_date<='".$to."') GROUP BY po.order_no  ORDER BY po.id DESC");
		   }else{
			$stmt = $conn->prepare("SELECT po.id AS item_id FROM purchase_order AS po LEFT JOIN materials AS mt ON mt.id = po.mat_id LEFT JOIN suppliers AS s ON s.id = po.sup_id LEFT JOIN sites AS st ON st.id = po.site_id LEFT JOIN users AS us ON us.id = po.user_id   GROUP BY po.order_no  ORDER BY po.id DESC");
			}							
            $stmt->execute();
		...
		return $setPaginate;
	       ...
	       if(isset($_GET['page'])){
		    ...
		    $stmt = $conn->prepare("SELECT po.id AS item_id,po.order_no, po.order_details, s.sup_name, po.order_date,po.order_status, st.site_name, us.fname FROM purchase_order AS po LEFT JOIN materials AS mt ON mt.id = po.mat_id LEFT JOIN suppliers AS s ON s.id = po.sup_id LEFT JOIN sites AS st ON st.id = po.site_id LEFT JOIN users AS us ON us.id = po.user_id  ".$filter_data." AND (po.order_date>='".$from."' AND po.order_date<='".$to."') GROUP BY po.order_no ORDER BY po.id DESC LIMIT ".$pageLimit.",".$setLimit."" );
			}else{
			$stmt = $conn->prepare("SELECT po.id AS item_id,po.order_no, po.order_details,s.sup_name, po.order_date,po.order_status, st.site_name, us.fname FROM purchase_order AS po LEFT JOIN materials AS mt ON mt.id = po.mat_id LEFT JOIN suppliers AS s ON s.id = po.sup_id LEFT JOIN sites AS st ON st.id = po.site_id LEFT JOIN users AS us ON us.id = po.user_id   GROUP BY po.order_no  ORDER BY po.id  DESC LIMIT ".$pageLimit.",".$setLimit."" );
			}						
$stmt->execute();
$displayed_records =0;
if($num_rows>=1){
      	...
      	<div>
            ...
            ?>
              <tr>  
				  ...
				  <td>
				  <div align="center"><a href="#" class="badge badge-success" data-bs-toggle="modal" data-bs-target="#ModalContentViewItem" onclick="viewDocument(<?php echo $row['order_no']; ?>)"> View</a> 
				  </div>
				  </td>
              </tr>   
            <?php
...
filter = <?php  echo $filter; ?>;
	...
	var myData = 'page=' + pageno + '&filter=' + filter;
     ...
     url: "views/reports/loads/load_purchase_order_data.php", //Where form data is sent on submission
...
to = "<?php  echo $to; ?>";
	// source of data	
	var myData = 'page=' + pageno + '&filter=' + filter + '&filter_form_data=' + filter_form_data  + '&supplier=' + supplier + '&site=' + site  + '&from=' + from  + '&to=' + to ;
	 jQuery.ajax({
     type: "GET", // Post / Get method
     url: "views/reports/loads/load_purchase_order_data.php", //Where form data is sent on submission
     dataType:"text", // Data type, HTML, json etc.
