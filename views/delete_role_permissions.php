...
try{	
   ...
   for($i=0;$i<=$count_items;$i++){
	   if(!empty($user_roles[$i])){		
	    $remove = $conn->prepare("DELETE FROM role_permissions WHERE id='".$user_roles[$i]."' LIMIT 1 ");
		$remove->execute();
	   }
	 ...
	 echo '<div class="alert alert-success d-flex align-items-center p-5 mb-10">
				...
				<h4 class="mb-1 text-success">Success</h4>
					<span>Permission(s) Removed</span>
				</div>
			</div>';
			 echo "<script>
		 ...
		 die("<div classs='alert alert-danger'> Database Error:-".$e->getMessage()."</div>");
	      ...
	      $log_desc ="Removed permissions  for  <strong>".$role_name."</strong>";
		  //Log Activity
