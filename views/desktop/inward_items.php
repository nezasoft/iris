...
?>
<table class="table table-striped table-bordered table-hover" id="inward_items">
                                    <thead>
                                        <tr>
										    <th>Item Code</th>
                                            <th>Del No</th>
									...
									?>
                                        <tr class="odd gradeX">
                                            <td><?php echo $qr_row['g_inward_id']; ?></td>                                       
                                            ...
                                            <td><?php echo titleCase($qr_row['g_desc']); ?></td>
  ...
  <script>
$('#inward_items').Tabledit({
    //Delete PO Item
    url: '../process/update_inward_details.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'id'], editable: [ [1, 'DelNo'], [2, 'VehNo'], [3, 'DelBy'], [5, 'Description'], [7, 'Quantity']]
    }
    , buttons: {
        edit: {
	...
	<a href="#" id="inw_ref_no" rel="<?php echo $_SESSION['COMSYS_InwardRefNo']; ?>"></a>
	<button class="btn btn-success" id="send_inw_f">Send Inward </button> <!--| <a class="btn btn-xs btn-danger" href="" id="clear_data">Clear Data </a> -->
    ...
    <div id="response2"></div>
      ...
      <script>
					...
					$(document).ready(function() {	
						$('.loader2').hide();					
					//send requisition
						$("#send_inw_f").click(function(){					
						$('.loader2').fadeIn();
						var inward_no_send = $("#inw_ref_no").attr("rel");
					
					    $("#response2").empty();
					    var con = confirm("Submit Inward?");
						 if(con==true){
						 $.post("../process/send_inward.php",
							{ inward_no_send: inward_no_send},
							function(inward_no_send) {
							$("#response2").html(inward_no_send);
							$('.loader2').hide();
							$('#TransDiv').hide();
