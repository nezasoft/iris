...
$get_req = $conn->prepare("SELECT tmp.id AS item_id, bs.section_name, w.work_unit_name, sb.sub_section_name, tmp.qty,tmp.req_ref_no, tmp.req_details,mt.mat_name, b.building_name, w.work_unit_name FROM temp_requisitions AS tmp LEFT JOIN materials AS mt ON mt.id = tmp.mat_id LEFT JOIN work_unit  AS w ON w.id = tmp.work_id LEFT JOIN sub_section AS sb ON sb.id = w.sub_section_id LEFT JOIN building_sections AS bs ON bs.id = sb.section_id  LEFT JOIN building AS b ON b.id = tmp.building_id WHERE tmp.req_ref_no='".$_SESSION['IRIS_MAT_REF_NO']."'  ORDER BY tmp.id ASC");
$get_req->execute();
...
$('#rq_items').Tabledit({
    //Delete PO Item
    url: 'process/update_material_requisition_item.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'id'], editable: [ [2, 'Quantity'],[7, 'Remarks']]
    }
   , buttons: {
        edit: {
