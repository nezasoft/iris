...
$get_req = $conn->prepare("SELECT bq.id AS item_id, bq.qty, bq.rate, bq.remark, m.mat_name, w.work_unit_name, p.pack_name FROM boq_temp AS bq LEFT JOIN materials AS m ON m.id = bq.item_id LEFT JOIN work_unit AS w ON w.id = bq.work_id LEFT JOIN packages AS p ON p.id = m.pack_id WHERE bq.ref_no='".$_SESSION['IRIS_BQ_REF_NO']."'");
$get_req->execute();
...
<thead style="background:#0291FA ;color:#fff;font-weight:600;">
  ...
  foreach($req_rows as $req_row){
	  $grand_total += $req_row['qty']*$req_row['rate'];
  ?>
  <tr>
    <td><?php echo $req_row['item_id']; ?></td>
	<td><?php echo titleCase($req_row['work_unit_name']); ?></td>
    <td><?php echo titleCase($req_row['mat_name']); ?></td>
    <td><?php echo $req_row['qty']; ?></td>
	<td><?php echo titleCase($req_row['pack_name']); ?></td>
    <td><?php echo $req_row['rate']; ?></td>
    <td><?php echo $req_row['remark']; ?></td>
	<td><?php echo number_format($req_row['qty']*$req_row['rate'],2,".",","); ?></td>
  </tr>
...
$('#rq_items').Tabledit({
    //Delete PO Item
    url: 'process/update_boq_item.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'id'], editable: [ [3, 'Quantity'],[5, 'Rate'],[6, 'Remarks']]
    }
   , buttons: {
        edit: {
