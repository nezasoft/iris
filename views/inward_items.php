...
<?php
//Get Inward details
$get_req = $conn->prepare("SELECT ti.id AS item_id, ti.qty, mt.mat_name FROM temp_inward AS ti LEFT JOIN materials AS mt ON mt.id = ti.mat_id   WHERE ti.ref_no='".$_SESSION['IRIS_INWARD_REF_NO']."' GROUP BY ti.mat_id");
$get_req->execute();
...
$('#rq_items').Tabledit({
    //Delete PO Item
    url: 'process/update_inward_item.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'id'], editable: [ [2, 'Quantity']]
    }
   , buttons: {
        edit: {
