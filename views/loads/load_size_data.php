...
exit();
      ...
      //check if user want all records or has wants a filtered result set
           ...
           $url = '?users&action=user_reports&';
			...
			$stmt = $conn->prepare("SELECT  id AS item_id FROM  sizes");
            $stmt->execute();
		...
		return $setPaginate;
            ...
            $pageLimit  = ($page * $setLimit) - $setLimit;
		      ...
		      $stmt = $conn->prepare("SELECT id AS item_id, size_name FROM sizes ORDER BY size_name ASC LIMIT ".$pageLimit.",".$setLimit."" );
              $stmt->execute();
	          $displayed_records=0;
			...
			<div class="table-responsive">
                <table class="table table-row-dashed table-striped table-row-gray-300 align-middle gs-0 gy-4" id="size_data" >
...
filter = <?php  echo $filter; ?>;
	...
	var myData = 'page=' + pageno + '&filter=' + filter;
     ...
     url: "views/loads/load_size_data.php", //Where form data is sent on submission
...
?>
  <script>
$('#size_data').Tabledit({
    url: 'process/update_size.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'id'], editable: [ [1, 'SizeName']]
    }
    , buttons: {
		<?php if($item94==1){ ?>
        edit: {
