    ...
    $store_id = $_GET['store_id'];
      //check if user want all records or has wants a filtered result set
           ...
           $url = '?users&action=user_reports&';
			...
			$stmt = $conn->prepare("SELECT st.id AS item_id   FROM materials_stock AS st LEFT JOIN materials AS mt ON mt.id = st.mat_id LEFT JOIN packages AS pk ON pk.id = st.pack_id WHERE st.store_id='".$store_id."'");
            $stmt->execute();
		...
		return $setPaginate;
            ...
            $pageLimit  = ($page * $setLimit) - $setLimit;
		      ...
		      $stmt = $conn->prepare("SELECT st.id AS item_id, st.min_stock_level,st.max_stock_level, st.quantity,st.stock_desc, mt.mat_name,pk.pack_name   FROM materials_stock AS st LEFT JOIN materials AS mt ON mt.id = st.mat_id LEFT JOIN packages AS pk ON pk.id = st.pack_id WHERE st.store_id='".$store_id."'  ORDER BY mt.mat_name ASC LIMIT ".$pageLimit.",".$setLimit."" );
              $stmt->execute();
	          $displayed_records=0;
			...
			<div class="table-responsive">
                <table class="table table-row-dashed table-striped table-row-gray-300 align-middle gs-0 gy-4" id="stock_data" >
...
store_id=<?php  echo $store_id; ?>;
	// source of data	
	var myData = 'page=' + pageno + '&filter=' + filter + '&store_id=' + store_id;
	 jQuery.ajax({
...
?>
 ...
 <script>
$('#stock_data').Tabledit({
    url: 'process/edit_stock_item.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'id'], editable: [ [2, 'StockDesc'],[5, 'MinLevel'],[6, 'MaxLevel']]
    }
    , buttons: {
        edit: {
