...
exit();
 ...
 //Get Sites
			  $sites = $conn->prepare("SELECT id AS site_id, site_name FROM sites  ORDER BY site_name  ASC");
			  $sites->execute();
			  $site_rows = $sites->fetchAll(PDO::FETCH_ASSOC);
              $site_count = $sites->rowCount();
			  //Get Roles
			  $roles = $conn->prepare("SELECT id AS role_id, role_name FROM roles ORDER BY role_name ASC");
			  $roles->execute();
			  $role_rows = $roles->fetchAll(PDO::FETCH_ASSOC);
			  $role_count = $roles->rowCount();
        ...
        if($site_count!=0){
				foreach($site_rows AS $site_row){ 
				 $sites .= '"'.$site_row['site_id'].'":  "'.titleCase($site_row['site_name']).'",';
				}
  		        $sites .='"'.$site_row['site_id'].'": "'.titleCase($site_row['site_name']).'"';                           
				$sites .="}'";
        }
        ...
        if($role_count!=0){
				foreach($role_rows as $role_row){
				 $roles .= '"'.$role_row['role_id'].'":  "'.titleCase($role_row['role_name']).'",';
				}
   	            $roles .='"'.$role_row['role_id'].'": "'.titleCase($role_row['role_name']).'"';                           
				$roles .="}'";
        }
      ...
      //check if user want all records or has wants a filtered result set
           ...
           $url = '?users&action=user_reports&';
			...
			$stmt = $conn->prepare("SELECT  us.id AS item_id FROM  users AS us LEFT JOIN sites AS s ON s.id = us.site_id LEFT JOIN roles AS r ON r.id = us.role_id ");
            $stmt->execute();
		...
		return $setPaginate;
            ...
            $pageLimit  = ($page * $setLimit) - $setLimit;
		      ...
		      $stmt = $conn->prepare("SELECT  us.id AS item_id, us.fname, us.lname,us.username, us.email,us.date_created,s.site_name, r.role_name, us.active FROM  users AS us LEFT JOIN sites AS s ON s.id = us.site_id LEFT JOIN roles AS r ON r.id = us.role_id ORDER BY us.fname ASC LIMIT ".$pageLimit.",".$setLimit."" );
              $stmt->execute();
	          $displayed_records=0;
			...
			<div class="table-responsive">
                <table class="table table-row-dashed table-striped table-row-gray-300 align-middle gs-0 gy-4" id="list_users" >
            ...
            ?>
              ...
              <td><?php echo $row['item_id']; ?></td>
				  ...
				  <td><?php echo titleCase($row['lname']); ?></td>
				  <td><?php echo $row['email']; ?></td>
				  <td><?php echo $row['username']; ?></td>
                  <td><?php echo titleCase($row['site_name']); ?></td>
				  <td><?php echo titleCase($row['role_name']); ?></td>
...
filter = <?php  echo $filter; ?>;
	...
	var myData = 'page=' + pageno + '&filter=' + filter;
     ...
     url: "views/loads/load_user_data.php", //Where form data is sent on submission
...
?>
  <script>
$('#list_users').Tabledit({
    url: 'process/update_user.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'id'], editable: [ [1, 'fName'],[2, 'sName'],[3, 'Email'],[4, 'Username'], [5, 'Site',<?php  echo $sites;?>],[6, 'Role',<?php  echo $roles;?>],[7, 'Status', '{"Yes": "Active", "No": "In-active"}']]
    }
    , buttons: {
		<?php if($item53==1){ ?>
        edit: {
