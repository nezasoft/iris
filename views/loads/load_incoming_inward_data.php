...
exit();
		   ...
		   if(isset($_GET['filter_form_data']) && sanitize_string($_GET['filter_form_data'])=='true'){
			...
			$stmt = $conn->prepare("SELECT gi.id AS item_id FROM good_inward AS gi LEFT JOIN materials AS mt ON mt.id = gi.mat_id LEFT JOIN users AS us ON us.id = gi.user_id LEFT JOIN suppliers AS s ON s.id = gi.sup_id   WHERE gi.site_id='".$_SESSION['IRIS_SITE_ID']."' AND (gi.inw_date>='".$from."' AND gi.inw_date<='".$to."') GROUP BY gi.inward_no  ORDER BY gi.id DESC");
		   }else{
			$stmt = $conn->prepare("SELECT gi.id AS item_id  FROM good_inward AS gi LEFT JOIN materials AS mt ON mt.id = gi.mat_id LEFT JOIN users AS us ON us.id = gi.user_id LEFT JOIN suppliers AS s ON s.id = gi.sup_id WHERE gi.site_id='".$_SESSION['IRIS_SITE_ID']."'   GROUP BY gi.inward_no  ORDER BY gi.id DESC");
			}							
            $stmt->execute();
		...
		return $setPaginate;
	       ...
	       if(isset($_GET['page'])){
			...
			$from = sanitize_string($_GET['from']); 
			$to = sanitize_string($_GET['to']); 
		    $stmt = $conn->prepare("SELECT gi.id AS item_id, gi.inward_no, gi.inw_date, gi.inw_time, gi.in_status, gi.delivery_note_no, gi.delivered_by, gi.g_desc, gi.vehicle_no, s.sup_name  FROM good_inward AS gi LEFT JOIN materials AS mt ON mt.id = gi.mat_id LEFT JOIN users AS us ON us.id = gi.user_id LEFT JOIN suppliers AS s ON s.id = gi.sup_id  WHERE gi.site_id='".$_SESSION['IRIS_SITE_ID']."'  AND (gi.inw_date>='".$from."' AND gi.inw_date<='".$to."') GROUP BY gi.inward_no ORDER BY gi.id DESC LIMIT ".$pageLimit.",".$setLimit."" );
			}else{
			$stmt = $conn->prepare("SELECT gi.id AS item_id, gi.inward_no, gi.inw_date, gi.inw_time, gi.in_status, gi.delivery_note_no, gi.delivered_by, gi.g_desc, gi.vehicle_no, s.sup_name  FROM good_inward AS gi LEFT JOIN materials AS mt ON mt.id = gi.mat_id LEFT JOIN users AS us ON us.id = gi.user_id LEFT JOIN suppliers AS s ON s.id = gi.sup_id  WHERE gi.site_id='".$_SESSION['IRIS_SITE_ID']."'    GROUP BY gi.inward_no  ORDER BY gi.id  DESC LIMIT ".$pageLimit.",".$setLimit."" );
			}						
$stmt->execute();
$displayed_records =0;
if($num_rows>=1){
      	...
      	<div>
            ...
            ?>
              <tr>  
				  ...
				  <td>
				  <?php if($item26==1){ ?>
				  <div align="center"><a href="#" class="badge badge-success" data-bs-toggle="modal" data-bs-target="#ModalContentViewItem" onclick="viewDocument(<?php echo $row['inward_no']; ?>)"> View</a> 
				  </div>
				  <?php } ?>
				  </td>
              </tr>   
...
filter = <?php  echo $filter; ?>;
	...
	var myData = 'page=' + pageno + '&filter=' + filter;
     ...
     url: "views/loads/load_incoming_inward_data.php", //Where form data is sent on submission
