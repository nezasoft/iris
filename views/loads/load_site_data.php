      ...
      //check if user want all records or has wants a filtered result set
		...
		return $setPaginate;
            ...
            $pageLimit  = ($page * $setLimit) - $setLimit;
		      ...
		      $stmt = $conn->prepare("SELECT id AS item_id, site_name,site_phone,site_email,site_postal,site_address,date_created,site_status,estimated_cost FROM sites ORDER BY site_name ASC LIMIT ".$pageLimit.",".$setLimit."" );
              $stmt->execute();
	          $displayed_records=0;
			...
			<div class="table-responsive">
              ...
              <td><?php echo $row['item_id']; ?></td>
				  ...
				  <td><?php echo titleCase($row['site_email']); ?></td>
				  <td><?php echo titleCase($row['site_phone']); ?></td>
				  <td><?php echo titleCase($row['site_postal']); ?></td>
				  <td><?php echo titleCase($row['site_address']); ?></td>
				  <td><?php echo titleCase($row['date_created']); ?></td>
				  <td><?php echo titleCase($row['site_status']); ?></td>
...
filter = <?php  echo $filter; ?>;
	...
	var myData = 'page=' + pageno + '&filter=' + filter;
     ...
     url: "views/loads/load_site_data.php", //Where form data is sent on submission
...
$('#list_table_data').Tabledit({
    url: 'process/update_site.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'id'], editable: [ [1, 'Name'],[2, 'Email'],[3, 'Phone'],[4, 'Postal'],[5, 'Physical'],[6, 'DateCreated'],[7, 'Status'],[8, 'Cost']]
    }
    , buttons: {
		<?php if($item62==1){ ?>
        edit: {
