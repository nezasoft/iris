...
exit();
      ...
      //check if user want all records or has wants a filtered result set
           ...
           $url = '?users&action=user_reports&';
			...
			$stmt = $conn->prepare("SELECT  id AS item_id FROM  building_sections");
            $stmt->execute();
		...
		return $setPaginate;
            ...
            $pageLimit  = ($page * $setLimit) - $setLimit;
		      ...
		      $stmt = $conn->prepare("SELECT id AS item_id, section_name FROM building_sections ORDER BY section_name ASC LIMIT ".$pageLimit.",".$setLimit."" );
              $stmt->execute();
	          $displayed_records=0;
...
filter = <?php  echo $filter; ?>;
	...
	var myData = 'page=' + pageno + '&filter=' + filter;
     ...
     url: "views/loads/load_section_data.php", //Where form data is sent on submission
...
$('#list_table_data').Tabledit({
    url: 'process/update_section.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'id'], editable: [ [1, 'SectionName']]
    }
    , buttons: {
		<?php if($item78==1){ ?>
        edit: {
