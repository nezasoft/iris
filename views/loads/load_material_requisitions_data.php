...
exit();
		   ...
		   if(isset($_GET['filter_form_data']) && sanitize_string($_GET['filter_form_data'])=='true'){
			...
			$stmt = $conn->prepare("SELECT req.id AS req_id  FROM materials_request_form AS req LEFT JOIN materials AS mt  ON mt.id = req.mat_id  LEFT JOIN building AS b ON b.id = req.building_id LEFT JOIN users AS us ON us.id = req.user_id WHERE req.site_id='".$_SESSION['IRIS_SITE_ID']."' ".$filter_t_type." AND (req.req_date>='".$from."' AND req.req_date<='".$to."') GROUP BY req.order_no  ORDER BY req.id DESC");
		   }else{
			$stmt = $conn->prepare("SELECT req.id AS req_id FROM materials_request_form AS req LEFT JOIN materials AS mt  ON mt.id = req.mat_id  LEFT JOIN building AS b ON b.id = req.building_id  LEFT JOIN users AS us ON us.id = req.user_id WHERE req.site_id='".$_SESSION['IRIS_SITE_ID']."' GROUP BY req.order_no  ORDER BY req.id DESC");
			}							
            $stmt->execute();
		...
		return $setPaginate;
	       ...
	       if(isset($_GET['page'])){
			...
			$stmt = $conn->prepare("SELECT req.id AS item_id, req.order_no, req.qty, req.req_status, req.req_desc, req.req_date, req.user_id, us.fname, b.building_name  FROM materials_request_form AS req LEFT JOIN materials AS mt  ON mt.id = req.mat_id LEFT JOIN building AS b ON b.id = req.building_id LEFT JOIN users AS us ON us.id = req.user_id WHERE req.site_id='".$_SESSION['IRIS_SITE_ID']."' GROUP BY req.order_no  ORDER BY req.id  DESC LIMIT ".$pageLimit.",".$setLimit."" );
			}						
$stmt->execute();
$displayed_records =0;
if($num_rows>=1){
?>
      	<div>
			<div class="table-responsive">
                <table class="table table-row-dashed table-striped table-row-gray-300 align-middle gs-0 gy-4" id="material_requisitions_data" >
            ...
            ?>
              <tr>  
				  ...
				  <td>
				  <?php if($item11==1){ ?>
				  <div align="center"><a href="#" class="badge badge-success" data-bs-toggle="modal" data-bs-target="#ModalContentViewItem" onclick="viewTransaction(<?php echo $row['order_no']; ?>)"> View</a> 
				  </div>
				  <?php } ?>
				  </td>
              </tr>   
...
filter = <?php  echo $filter; ?>;
	...
	var myData = 'page=' + pageno + '&filter=' + filter;
     ...
     url: "views/loads/load_material_requisitions_data.php", //Where form data is sent on submission
...
?>
<script>
$("#material_requisitions_data").tableExport({
  ...
  sheetname: "Material Requisitions Report" 
