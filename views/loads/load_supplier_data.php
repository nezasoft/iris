...
exit();
      ...
      //check if user want all records or has wants a filtered result set
           ...
           $url = '?users&action=user_reports&';
			...
			$stmt = $conn->prepare("SELECT  id AS item_id FROM suppliers ");
            $stmt->execute();
		...
		return $setPaginate;
            ...
            $pageLimit  = ($page * $setLimit) - $setLimit;
		      ...
		      $stmt = $conn->prepare("SELECT id AS item_id, sup_name,sup_email,sup_phone,sup_postal,sup_physical FROM suppliers ORDER BY sup_name ASC LIMIT ".$pageLimit.",".$setLimit."" );
              $stmt->execute();
	          $displayed_records=0;
			...
			<div class="table-responsive">
                <table class="table table-row-dashed table-striped table-row-gray-300 align-middle gs-0 gy-4" id="supplier_data" >
            ...
            ?>
              ...
              <td><?php echo $row['item_id']; ?></td>
				  ...
				  <td><?php echo $row['sup_postal']; ?></td>
				  <td><?php echo $row['sup_physical']; ?></td>
              </tr>   
            <?php
            }
...
filter = <?php  echo $filter; ?>;
	...
	var myData = 'page=' + pageno + '&filter=' + filter;
     ...
     url: "views/loads/load_supplier_data.php", //Where form data is sent on submission
...
?>
  <script>
$('#supplier_data').Tabledit({
    url: 'process/update_supplier.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'id'], editable: [ [1, 'SupName'],[2, 'SupEmail'],[3, 'SupPhone'],[4, 'SupPostal'],[6, 'SupPhysical']]
    }
    , buttons: {
		<?php if($item43==1){ ?>
        edit: {
