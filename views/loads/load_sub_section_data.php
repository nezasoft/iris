...
exit();
      ...
      //check if user want all records or has wants a filtered result set
           ...
           $url = '?users&action=user_reports&';
			...
			$stmt = $conn->prepare("SELECT ss.id AS item_id FROM sub_section AS ss LEFT JOIN building_sections AS bs ON bs.id = ss.section_id");
            $stmt->execute();
		...
		return $setPaginate;
            ...
            $pageLimit  = ($page * $setLimit) - $setLimit;
		      ...
		      $stmt = $conn->prepare("SELECT ss.id AS item_id, ss.sub_section_name, bs.section_name FROM sub_section AS ss LEFT JOIN building_sections AS bs ON bs.id = ss.section_id ORDER BY bs.section_name ASC LIMIT ".$pageLimit.",".$setLimit."" );
              $stmt->execute();
	          $displayed_records=0;
...
filter = <?php  echo $filter; ?>;
	...
	var myData = 'page=' + pageno + '&filter=' + filter;
     ...
     url: "views/loads/load_sub_section_data.php", //Where form data is sent on submission
...
<?php
  ...
  //Get Sections
$sec = $conn->prepare("SELECT id AS section_id, section_name FROM building_sections  ORDER BY section_name  ASC");
$sec->execute();
$sec_rows = $sec->fetchAll(PDO::FETCH_ASSOC);
$sec_count = $sec->rowCount();
$sec = "'{";
  if($sec_count!=0){
	foreach($sec_rows AS $sec_row){ 
	$sec .= '"'.$sec_row['section_id'].'":  "'.titleCase($sec_row['section_name']).'",';
	}
  	$sec .='"'.$sec_row['section_id'].'": "'.titleCase($sec_row['section_name']).'"';                           
	$sec .="}'";
   }
?>
  <script>
$('#list_table_data').Tabledit({
    url: 'process/update_sub_section.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'id'], editable: [ [1, 'SubSection'],[2, 'Section',<?php  echo $sec;?>]]
    }
    , buttons: {
		<?php if($item82==1){ ?>
        edit: {
