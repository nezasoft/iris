...
exit();
      ...
      //check if user want all records or has wants a filtered result set
           ...
           $url = '?users&action=user_reports&';
			...
			$stmt = $conn->prepare("SELECT  id AS item_id FROM stores WHERE site_id='".$_SESSION['IRIS_SITE_ID']."'");
            $stmt->execute();
		...
		return $setPaginate;
            ...
            $pageLimit  = ($page * $setLimit) - $setLimit;
		      $stmt = $conn->prepare("SELECT id AS item_id, store_name,store_desc FROM stores WHERE site_id='".$_SESSION['IRIS_SITE_ID']."' ORDER BY store_name ASC LIMIT ".$pageLimit.",".$setLimit."" );
              $stmt->execute();
	          $displayed_records=0;
            ...
            ?>
              ...
              <td><?php echo $row['item_id']; ?></td>
				  ...
				  <td><?php echo titleCase($row['store_desc']); ?></td>
				  <td><div align="right"><a href="#" class="btn btn-xs btn-sm btn-info" onclick="viewStore(<?php echo $row['item_id']; ?>)"> View</a> 
...
$('#list_table_data').Tabledit({
    url: 'process/update_store.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'id'], editable: [ [1, 'Name'],[2, 'Desc']]
    }
    , buttons: {
        edit: {
