...
include('../../connect_db/connect_db.php');
		   ...
		   if(($_SESSION['COMSYS_UserRole']=='CompanyAdmin') || ($_SESSION['COMSYS_UserRole']=='LPOManager')){
		     $filter_val= "tdn.from_site_id like '%%'";
		   }else{
		     $filter_val = "tdn.from_site_id='".$_SESSION['COMSYS_SiteID']."'";
		   }
           if(isset($_GET['filter']) && sanitize_string($_GET['filter'])){
           $val = sanitize_string($_GET['filter']);
           $filter = 'LIMIT '.$val;
	
           $url = '?acc_type='.$_SESSION['COMSYS_AccountType'].'&site_id='.$_SESSION['COMSYS_SiteID'].'&sec=del&act=man_del&filter='.$val.'#man_del&';
           }else{
           $val = 30;
           $filter = '';
           $url = '?acc_type='.$_SESSION['COMSYS_AccountType'].'&site_id='.$_SESSION['COMSYS_SiteID'].'&sec=del&act=man_del&';
           }
			$qr= mysql_query("select tdn.delivery_no from delivery_note as tdn left join materials as mt on mt.mat_id = tdn.mat_id left join packages as pk on pk.pack_id = tdn.pack_id left join users as usatn on usatn.user_id = tdn.attn_user_id left join sites as s on s.site_id=tdn.to_site_id 
			...
			where ".$filter_val." group by tdn.delivery_no 
			order by tdn.delivery_id desc ");
		...
		return $setPaginate;
	       ...
	       if(isset($_GET['page'])){
			...
			$qr=mysql_query("select site_from.site_name as site_name_from, tdn.delivery_no,tdn.delivery_id,tdn.delivery_status,tdn.del_date,tdn.del_time, tdn.vehicle_no,tdn.taken_by, usatn.fname,usatn.lname,usatn.user_id, s.site_id,s.site_name,pk.pack_id, tdn.delivery_id, tdn.qty, tdn.mat_id,tdn.del_details, mt.mat_name,pk.pack_name from delivery_note as tdn left join materials as mt on mt.mat_id = tdn.mat_id 
			left join packages as pk on pk.pack_id = tdn.pack_id 
			left join users as usatn on usatn.user_id = tdn.attn_user_id 
			left join sites as s on s.site_id=tdn.to_site_id 
			left join users as user_from on user_from.user_id = tdn.user_id
			left join sites as site_from on site_from.site_id = user_from.site_id 
			where ".$filter_val." group by tdn.delivery_no 
			order by tdn.delivery_id desc limit ".$pageLimit.",".$setLimit." ");					
			$displayed_records=0;
	        $count = 0;
	        if($num_rows>=1){		
             ?>
                               ...
                               <table class="table table-striped table-bordered table-hover" id="list_del">
                                    <thead>
                                        <tr>
									...
									?>
                                        <tr class="odd gradeX">
                                         <td><?php echo $qr_row['delivery_no']; ?></td>
                                          <td><?php echo $qr_row['taken_by']; ?></td>
										  <td><?php echo $qr_row['vehicle_no']; ?></td>
										  <td><?php echo $qr_row['site_name_from']; ?></td>
										  <td><?php echo $qr_row['site_name']; ?></td>
										  <td><?php echo '<font color="'.$color_code.'">'. $qr_row['delivery_status'].'</font>';
												
											?></td>
 
                                            <td><?php echo $qr_row['del_date']; ?></td>
                                            <td><?php echo $qr_row['del_time']; ?></td>
								            <td><a class="btn btn-xs btn-success" href="?acc_type=<?php echo $_SESSION['COMSYS_AccountType']; ?>&site_id=<?php echo $_SESSION['COMSYS_SiteID']; ?>&sec=del&act=view_del&del_ref=<?php echo $qr_row['delivery_no']; ?>">View</a></td>
                                        </tr>
								 <?php
								 }
								 ?>
 ...
 <ul style="background-color:transparent; border-color:transparent;" class="pagination pagination-sm m-t-0 m-b-0 pull-right" id="paginated_data">
	 ...
	 <?php  
		...
		echo '<div class="alert alert-close alert-danger"><div class="alert-content"></div><font color="red"><i class="glyph-icon icon-check-circle"></i></font> There are currently no records!</div>';
         ...
         ?>
 <br/><br/><br/><br/><br/><br/><br/><div id="rec_found" class="alert alert-success"><font color="green"><i class="glyph-icon icon-check-circle"></i></font>&nbsp;<?php  echo "Displaying <strong>".$count."</strong> records out of total <strong>".$num_rows."</strong> found ";?></div>
...
filter = <?php  echo $filter; ?>;
	...
	var myData = 'page=' + pageno + '&filter=' + filter;
	 jQuery.ajax({
                ...
                url: "../views/loads/load_delivery_data.php", //Where form data is sent on submission
                dataType:"text", // Data type, HTML, json etc.
                data:myData, //Form variables
                success:function(response){
                $("#delivery_data").html(response);
				$("#loader").hide();     
                }
            });
}
