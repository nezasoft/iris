...
include('../../connect_db/connect_db.php');
           ...
           $url = '?acc_type='.$_SESSION['COMSYS_AccountType'].'&site_id='.$_SESSION['COMSYS_SiteID'].'&sec=st&act=qs&filter='.$val.'#list_stores&';
           }else{
           $val = 30;
           $filter = '';
           $url = '?acc_type='.$_SESSION['COMSYS_AccountType'].'&site_id='.$_SESSION['COMSYS_SiteID'].'&sec=st&act=qs#list_stores';
           }
		...
		return $setPaginate;
	       ...
	       if(isset($_GET['page'])){
			...
			$qr=mysql_query("select ms.stock_desc,ms.min_stock_level,ms.max_stock_level,ms.quantity,ms.add_date,ms.add_time,mt.mat_name,pk.pack_name, st.store_name from materials_stock as ms 
			left join stores as st on st.store_id = ms.store_id
			left join materials as mt on mt.mat_id = ms.mat_id
			left join packages as pk on pk.pack_id = ms.pack_id
			".$filter_val." order by mt.mat_name limit ".$pageLimit.",".$setLimit." ");
			$displayed_records=0;
	        $count = 0;
	      if($num_rows>=1){
             ...
             ?>
						...
						<h3><a href="#" onClick ="$('#list_stock').tableExport({type:'excel',escape:'false'});" >Export To Excel</a></h3>
                           <div class="panel-body">
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered table-hover" id="list_stock">
                                    <thead>
                                        <tr>
                                            <th>Material Name</th>
                                            <th>Package</th>
									...
									?>
                                       ...
                                       echo '<font color="blue">Excess Stock</font>';
                                         ...
                                         ?></td>
                                       <td><?php echo titleCase($qr_row['store_name']); ?></td>
...
filter = <?php  echo $filter; ?>;
	...
	var myData = 'page=' + pageno + '&filter=' + filter ;
	 jQuery.ajax({
                ...
                url: "../views/loads/quick_search_data.php", //Where form data is sent on submission
                dataType:"text", // Data type, HTML, json etc.
                data:myData, //Form variables
                success:function(response){
                $("#quick_search_data").html(response);
				$("#loader").hide();     
                }
            });
}
