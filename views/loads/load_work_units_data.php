...
exit();
      ...
      //check if user want all records or has wants a filtered result set
           ...
           $url = '?users&action=user_reports&';
			...
			$stmt = $conn->prepare("SELECT w.id AS item_id FROM work_unit AS w LEFT JOIN sub_section AS s ON s.id = w.sub_section_id LEFT JOIN building_sections AS bs ON bs.id = s.section_id");
            $stmt->execute();
		...
		return $setPaginate;
            ...
            $pageLimit  = ($page * $setLimit) - $setLimit;
		      ...
		      $stmt = $conn->prepare("SELECT w.id AS item_id, w.work_unit_name, s.sub_section_name, bs.section_name FROM work_unit AS w LEFT JOIN sub_section AS s ON s.id = w.sub_section_id LEFT JOIN building_sections AS bs ON bs.id = s.section_id ORDER BY bs.id ASC LIMIT ".$pageLimit.",".$setLimit."" );
              $stmt->execute();
	          $displayed_records=0;
			...
			<div class="table-responsive">
                <table class="table table-row-dashed table-striped table-row-gray-300 align-middle gs-0 gy-4" id="work_unit_data" >
            ...
            ?>
			  ...
			  <td><?php echo titleCase($row['sub_section_name']); ?></td>
...
filter = <?php  echo $filter; ?>;
	...
	var myData = 'page=' + pageno + '&filter=' + filter;
     ...
     url: "views/loads/load_work_units_data.php", //Where form data is sent on submission
...
?>
  <script>
$('#work_unit_data').Tabledit({
    url: 'process/update_work_unit.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'id'], editable: [ [1, 'WorkUnit']]
    }
    , buttons: {
		<?php if($item86==1){ ?>
        edit: {
