...
exit();
      ...
      //check if user want all records or has wants a filtered result set
           ...
           $url = '?users&action=user_reports&';
			...
			$stmt = $conn->prepare("SELECT  b.id AS item_id FROM building AS b LEFT JOIN sites AS s ON s.id = b.site_id");
            $stmt->execute();
		...
		return $setPaginate;
            ...
            $pageLimit  = ($page * $setLimit) - $setLimit;
		      ...
		      $stmt = $conn->prepare("SELECT b.id AS item_id, building_name,site_name, b.status FROM building AS b LEFT JOIN sites AS s ON s.id = b.site_id ORDER BY b.building_name ASC LIMIT ".$pageLimit.",".$setLimit."" );
              $stmt->execute();
	          $displayed_records=0;
			...
			<div class="table-responsive">
                <table class="table table-row-dashed table-striped table-row-gray-300 align-middle gs-0 gy-4" id="housing_unit_data" >
            ...
            ?>
              <tr>  
				  ...
				  <td><a href="#" class="badge badge-info" onclick="viewUnit(<?php echo $row['item_id']; ?>)" data-bs-toggle="modal" data-bs-target="#ModalContentView" > View </a></td>
              </tr>   
            <?php
            }
...
?>
  ...
  <script>
$('#housing_unit_data').Tabledit({
    url: 'process/update_unit.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'id'], editable: [ [1, 'UnitName'],[2, 'Site',<?php  echo $sites;?>],[3, 'Status','{"N":"New","C":"Completed","S":"Sold"}']]
    }
    , buttons: {
		<?php if($item74==1){ ?>
        edit: {
