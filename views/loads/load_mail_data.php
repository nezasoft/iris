...
exit();
      ...
      //check if user want all records or has wants a filtered result set
           ...
           $url = '?users&action=user_reports&';
			...
			$stmt = $conn->prepare("SELECT  id AS item_id FROM mail_setting");
            $stmt->execute();
		...
		return $setPaginate;
            ...
            $pageLimit  = ($page * $setLimit) - $setLimit;
		      ...
		      $stmt = $conn->prepare("SELECT id AS item_id, mailer_name, mail_url,incoming_port, outgoing_port, email, username,password FROM mail_setting ORDER BY mailer_name ASC LIMIT ".$pageLimit.",".$setLimit."" );
              $stmt->execute();
	          $displayed_records=0;
            ...
            ?>
              ...
              <td><?php echo $row['item_id']; ?></td>
				  <td><?php echo titleCase($row['mailer_name']); ?></td>
				  <td><?php echo $row['mail_url']; ?></td>
				  <td><?php echo $row['incoming_port']; ?></td>
				  <td><?php echo $row['outgoing_port']; ?></td>
...
filter = <?php  echo $filter; ?>;
	...
	var myData = 'page=' + pageno + '&filter=' + filter;
     ...
     url: "views/loads/load_mail_data.php", //Where form data is sent on submission
...
$('#list_table_data').Tabledit({
    url: 'process/update_mail.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'id'], editable: [ [1, 'Name'],[2, 'URL'],[3, 'Incoming'],[4, 'Outgoing'],[5, 'Email'],[6, 'Username'],[7, 'Password']]
    }
    , buttons: {
        edit: {
