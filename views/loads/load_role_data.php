...
exit();
      ...
      //check if user want all records or has wants a filtered result set
           ...
           $url = '?users&action=user_reports&';
			...
			$stmt = $conn->prepare("SELECT  id AS item_id FROM  roles");
            $stmt->execute();
		...
		return $setPaginate;
            ...
            $pageLimit  = ($page * $setLimit) - $setLimit;
		      ...
		      $stmt = $conn->prepare("SELECT id AS item_id, role_name FROM roles ORDER BY role_name ASC LIMIT ".$pageLimit.",".$setLimit."" );
              $stmt->execute();
	          $displayed_records=0;
			...
			<div class="table-responsive">
                <table class="table table-row-dashed table-striped table-row-gray-300 align-middle gs-0 gy-4" id="list_table_data" >
                    <thead class="fw-bolder text-muted bg-light-dark">
                      <tr >
                        <th>#</th>
                        <th>Role</th>
				        <th></th>
            ...
            ?>
              ...
              <td><?php echo $row['item_id']; ?></td>
				  ...
				  <td><a  href="#" class="badge badge-danger" data-bs-toggle="modal" data-bs-target="#ModalContentAddNew" onclick="assignRole(<?php echo $row['item_id']; ?>)" >Add Permission</a> 
				 <a  href="#"   data-bs-toggle="modal" data-bs-target="#ModalContentAddNew" onclick="viewRole(<?php echo $row['item_id']; ?>)" class="badge badge-primary" >View Permission</a>
				  </div>
				  </td>
              </tr>   
            <?php
...
filter = <?php  echo $filter; ?>;
	...
	var myData = 'page=' + pageno + '&filter=' + filter;
     ...
     url: "views/loads/load_role_data.php", //Where form data is sent on submission
...
$('#list_table_data').Tabledit({
    url: 'process/update_role.php', editButton: true, removeButton: true, columns: {
        identifier: [0, 'id'], editable: [ [1, 'RoleName']]
    }
    , buttons: {
		<?php if($item57==1){ ?>
        edit: {
