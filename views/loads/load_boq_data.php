...
exit();
      ...
      //check if user want all records or has wants a filtered result set
           ...
           $url = '?users&action=user_reports&';
			...
			$stmt = $conn->prepare("SELECT bq.id AS item_id FROM boq AS bq LEFT JOIN sites AS s ON s.id = bq.site_id LEFT JOIN users AS us ON us.id = bq.user_id");
            $stmt->execute();
		...
		return $setPaginate;
            ...
            $pageLimit  = ($page * $setLimit) - $setLimit;
		      ...
		      $stmt = $conn->prepare("SELECT bq.id AS item_id, bq.boq_no, bq.boq_name, bq.boq_date, bq.boq_time, bq.units, s.site_name, us.fname FROM boq AS bq LEFT JOIN sites AS s ON s.id = bq.site_id LEFT JOIN users AS us ON us.id = bq.user_id ORDER BY bq.id DESC LIMIT ".$pageLimit.",".$setLimit."" );
              $stmt->execute();
	          $displayed_records=0;
            ...
            ?>
			  ...
			  <td><?php echo $row['boq_no']; ?></td>
			  <td><?php echo $row['boq_name']; ?></td>
				  <td><?php echo titleCase($row['site_name']); ?></td>
				  <td><?php echo $row['units']; ?></td>
				  <td><?php echo $row['boq_date']; ?></td>
				  <td><?php echo titleCase($row['fname']); ?></td>
				  <td><div align="center">
				  <?php if($item90==1){ ?>
				  <a href="#" class="badge badge-warning" data-bs-toggle="modal" data-bs-target="#ModalContentEditBOQ" onclick="editBOQ(<?php echo $row['item_id']; ?>)"> Edit</a> 
				  <?php } ?>
				  <?php if($item91==1){ ?>
				  <a href="#" class="badge badge-success" data-bs-toggle="modal" data-bs-target="#ModalContentViewBOQ" onclick="viewBOQ(<?php echo $row['item_id']; ?>)"> View</a>  
				  <?php } ?>
...
filter = <?php  echo $filter; ?>;
	...
	var myData = 'page=' + pageno + '&filter=' + filter;
     ...
     url: "views/loads/load_boq_data.php", //Where form data is sent on submission
